...
public class ParquetTest {
    ...
    public void testParquetFile() throws IOException {
        ...
        ParquetReader<Group> reader = new ParquetReader<>(path,
                readSupport);
        Group result = reader.read();
        assertNotNull(result);
        assertThat(result.getString("left", 0), is("L"));
        assertThat(result.getString("right", 0),
                is("R"));
    ...
    @Test
    public void testParquetFileUsingAvroProjectionAndReadSchema()
            throws IOException {
        Schema.Parser parser = new Schema.Parser();
        Schema schema = parser.parse(getClass().getResourceAsStream(
                "StringPair.avsc"
        ...
        parser = new Schema.Parser();
        Schema projectionSchame = parser.parse(
                getClass().getResourceAsStream("ProjectedStringPair.avsc"));
        parser = new Schema.Parser();
        Schema readSchema = parser.parse(
                getClass().getResourceAsStream("NewStringPair.avsc"));
        Configuration conf = new Configuration();
        AvroReadSupport.setRequestedProjection(conf, projectionSchame);
